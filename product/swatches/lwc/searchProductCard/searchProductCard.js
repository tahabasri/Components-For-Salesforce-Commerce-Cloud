...
import getActiveColors from '@salesforce/apex/SwatchController.getActiveColors';
...
export default class SearchProductCard extends LightningElement {
    ...
    set displayData(data) {
        ...
        if (this._displayData.productClass == 'VariationParent') {
            ...
            .then(results => {
                this._colorToProductIdMap = results;
            }).catch(error => {
    ...
    get fields() {
        return (this.displayData?.fields ?? []).map((field) => {
            let isSwatchConfig = field.name === 'SwatchConfig__c';
            let swatchItems;
            if (isSwatchConfig) {
                let colorSwatches = JSON.parse(field.value).Color;
                let swatchConfig = Object.keys(colorSwatches);
                swatchItems = swatchConfig
                .filter(color => {
                    return Object.keys(this._colorToProductIdMap).includes(color);
                })
                .map(color => {
                    return  {
                        label: color,
    ...
    handleOnChanged(e) {
        let selectedColor = this.querySelector('c-swatch-selector').value;
        let variantProductId = this._colorToProductIdMap[selectedColor];
        navigate(this._navigationContext, {
            type: 'standard__recordPage',
            attributes: {
                objectApiName: 'Product2',
