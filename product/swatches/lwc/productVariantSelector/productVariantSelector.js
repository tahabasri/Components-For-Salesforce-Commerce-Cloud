...
export default class ProductVariantSelector extends LightningElement {
    ...
    set product(val) {
        ...
        this._swatchConfig = JSON.parse(val?.fields?.SwatchConfig__c || '{}');
    }
    ...
    get currentlySelectedOptions() {
        ...
        let swatchItems = [...this.querySelectorAll('c-swatch-selector')].map((item) => item.value);
        return [...items,...swatchItems];
    }
    ...
    get normalizedVariants() {
        const swatchAttributes = Object.keys(this._swatchConfig);

        return (this.variants || []).map((variant, variantIndex) => {
            const isSwatchAttribute = swatchAttributes.includes(variant.label);
            ...
            return {
                ...
                options: variant.options.map((option) => {
                    ...
                    if (isSwatchAttribute) {
                        item = {...item, url: this._swatchConfig[variant.label][option.value]};
                    }
                    return item;
                }),
            };
